...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Navier-Stokes PINN, full-observation + full-collocation:"
                    " data and PDE both use all 1M points, with no random sampling."
    )
    ...
    # Adam early stopping: stop if total loss does not decrease for N consecutive steps,
    # and roll the model back to the best state seen. patience = 0 disables it
    # (old behavior: run the full adam_steps).
    ...
    # lr schedule: cosine annealing smoothly decays from adam_lr to lr_eta_min over a period of adam_steps.
    # 'none' means a fixed lr.
    ...
    # Observation corruption (added to u, v only; p has no observations)
...
def build_observation_uv_np(flat_reference, seed, noise_level, bias_level, bias_type):
    """uv-only observation corruption: noise / bias is added to u, v only; p stays clean (it is never supervised)."""
    ...
    p = flat_reference["p"].astype("float32").copy()  # kept for diagnostics, never enters the loss
...
def make_observation_cache_name(*, seed, noise_level, bias_level, bias_type):
    parts = [f"seed_{seed}", f"noise_gaussian_uv_{format_level_for_name(noise_level)}"]
    if bias_level > 0:
        parts.append(f"bias_{bias_type}_uv_{format_level_for_name(bias_level)}")
...
def get_observation_dataset(flat_reference, seed, noise_level, bias_level, bias_type, refresh_cache):
    ...
    if cp.exists() and not refresh_cache:
        ...
        flat = build_observation_uv_np(flat_reference, seed, noise_level, bias_level, bias_type)
...
def build_full_batch(pool):
    """x/y/t are made grad leaves (autograd differentiates w.r.t. them); u/v are the supervision targets (if present in the pool)."""
...
def model_uv_output(model, x, y, t):
    psi, _ = model(x, y, t)
    return grad(psi, y)[0], -grad(psi, x)[0]
...
def total_loss(model, data_pts, pde_pts, lw):
    u_p, v_p = model_uv_output(model, data_pts["x"], data_pts["y"], data_pts["t"])
    L_data = sse(u_p, data_pts["u"]) + sse(v_p, data_pts["v"])
...
def save_history_plot(history, out_path):
    ...
    ax.plot(x, [r["loss_data"] for r in history], label="data (uv only)")
...
def save_slice_plot(ref, flat_obs, pred_uvp, plot_t_idx, out_path):
    ...
    fig.suptitle(f"uv-fulltime (full-obs, full-collocation) @ t_idx={plot_t_idx}")
    fields = [("u", u_ref, u_obs, u_pred), ("v", v_ref, v_obs, v_pred), ("p", p_ref, None, p_pred)]
...
def save_rel_l2_vs_t_plot(rel_u_bins, rel_v_bins, rel_p_bins, ref_t, out_path):
    ...
    ax.set_title("uv-fulltime (full-obs, full-collocation): rel_l2 along time horizon")
...
def scenario_id(noise_level, bias_level, bias_type):
    parts = []
    if noise_level > 0: parts.append(f"noise_uv_{format_level_for_name(noise_level)}")
    if bias_level > 0:  parts.append(f"bias_{bias_type}_uv_{format_level_for_name(bias_level)}")
...
def main():
    ...
    print(f"device={device}, data=full {data_pool_size}, pde=full {pde_pool_size}, "
    ...
    metrics = {
        "setup": "uv_fullobs_full_collocation",
        "data_targets": "uv",
        ...
        "noise_targets": ["u", "v"], "bias_targets": ["u", "v"] if args.bias_level > 0 else [],
